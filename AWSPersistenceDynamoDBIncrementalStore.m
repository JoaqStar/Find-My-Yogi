...
- (id)executeRequest:(NSPersistentStoreRequest *)request withContext:(NSManagedObjectContext *)context error:(NSError **)error
{
    ...
    if (request.requestType == NSFetchRequestType)
    {
        ...
        if (fetchRequest.resultType == NSManagedObjectResultType)
        {
            ...
            // HashKey getItem if it is only a hash file (no range key)
            else if([fetchRequest.predicate isKindOfClass:[NSComparisonPredicate class]] && entityRangeKeyName == nil)
            {
            ...
            else if([fetchRequest.predicate isKindOfClass:[NSCompoundPredicate class]])
            {
                   ...
                   && [[predicate subpredicates] count] == 2)
                {
                       ...
                       && [[[rightPredicate leftExpression] keyPath] isEqualToString:entityRangeKeyName])
                    {
                           ...
                           && rightPredicate.predicateOperatorType == NSEqualToPredicateOperatorType)
                        {
                            ...
                            NSMutableArray *getItemResult = [self getItem:fetchRequest withContext:context withHashKey:hashKeyValue withRangeKey:rangeKeyValue error:error];
                            ...
                            NSMutableArray *getItemResult = [self query:fetchRequest withContext:context withHashKey:hashKeyValue withRangeKey:rangeKeyValue rangeKeyOperator:rightPredicate.predicateOperatorType sortAscending:sortAscending limit:limit error:error];
                ...
                id hashKeyValue = [[predicate rightExpression] expressionValueWithObject:nil context:nil];
                
                uint limit = 100;
                BOOL sortAscending = YES;
                
                NSMutableArray *getItemResult = [self query:fetchRequest withContext:context withHashKey:hashKeyValue sortAscending:sortAscending limit:limit error:error];
...
- (NSMutableArray *)getItem:(NSFetchRequest *)request withContext:(NSManagedObjectContext *)context withHashKey:(id)hashKey withRangeKey:(id)rangeKey error:(NSError **)error
{
    ...
    @try
    {
        ...
        DynamoDBAttributeValue *hashAttributeValue = [self attributeValueFromObject:hashKey];
        NSMutableDictionary *key = [NSMutableDictionary dictionaryWithObject:hashAttributeValue
                                                                      forKey:[self hashKeyForEntityName:request.entity.name]];

        if(rangeKey != nil)
        {
            DynamoDBAttributeValue *rangeAttributeValue = [self attributeValueFromObject:rangeKey];

            [key addEntriesFromDictionary:[NSMutableDictionary dictionaryWithObject:rangeAttributeValue
                                                                             forKey:[self rangeKeyForEntityName:request.entity.name]]];
        ...
        if(getItemResponse.error == nil)
        {
            if([getItemResponse.item count] > 0)
            {
                DynamoDBAttributeValue *attributeHashValue = (DynamoDBAttributeValue *)[getItemResponse.item objectForKey:[self hashKeyForEntityName:request.entity.name]];
                NSString *hashKey;

                if(attributeHashValue.s != nil)
                {
                    hashKey = attributeHashValue.s;
                }
                else if(attributeHashValue.n != nil)
                {
                    hashKey = attributeHashValue.n;
                }

                DynamoDBAttributeValue *attributeValueRangeKey = (DynamoDBAttributeValue *)[getItemResponse.item objectForKey:[self rangeKeyForEntityName:request.entity.name]];
                NSString *rangeKey;
                
                if(attributeValueRangeKey.s != nil)
                {
                    rangeKey = attributeValueRangeKey.s;
                }
                else if(attributeValueRangeKey.n != nil)
                {
                    rangeKey = attributeValueRangeKey.n;
                }
                
                NSDictionary *attributeClasses = [self attributeClassesForClassName:request.entityName];
                id hashKeyObject = [self convertString:hashKey toClass:[attributeClasses objectForKey:[self hashKeyForEntityName:request.entity.name]]];
                id rangeKeyObject = [self convertString:rangeKey toClass:[attributeClasses objectForKey:[self rangeKeyForEntityName:request.entity.name]]];
                ...
                NSManagedObjectID *objectId = [self newObjectIDForEntity:request.entity
                                                         referenceObject:[self referenceObjectIdForCoreData:key]];
...
- (NSMutableArray *)query:(NSFetchRequest *)request withContext:(NSManagedObjectContext *)context withHashKey:(id)hashKey sortAscending:(BOOL)ascending limit:(int)limit error:(NSError **)error
{
    return [self query:request withContext:context withHashKey:hashKey withRangeKey:nil rangeKeyOperator:NSEqualToPredicateOperatorType sortAscending:ascending limit:limit error:error];
}

- (NSMutableArray *)query:(NSFetchRequest *)request withContext:(NSManagedObjectContext *)context withHashKey:(id)hashKey withRangeKey:(id)rangeKey rangeKeyOperator:(NSPredicateOperatorType)operatorType sortAscending:(BOOL)ascending limit:(int)limit error:(NSError **)error
{
    ...
    @try
    {
        ...
        queryRequest.tableName = [self tableNameForEntityName:request.entityName];
        ...
        [condition addAttributeValueList:attributeValueHashKeyValue];
        queryRequest.keyConditions = [NSMutableDictionary dictionaryWithObject:condition forKey:[self hashKeyForEntityName:request.entity.name]];
        
        if (rangeKey != nil)
        {
            DynamoDBCondition *rangeCondition = [DynamoDBCondition new];
            DynamoDBAttributeValue *attributeValueRangeKeyValue = [self attributeValueFromObject:rangeKey];
            switch (operatorType) {
                case NSEqualToPredicateOperatorType:
                    rangeCondition.comparisonOperator = @"EQ";
                    break;
                case NSGreaterThanPredicateOperatorType:
                    rangeCondition.comparisonOperator = @"GT";
                    break;
                case NSGreaterThanOrEqualToPredicateOperatorType:
                    rangeCondition.comparisonOperator = @"GE";
                    break;
                case NSLessThanPredicateOperatorType:
                    rangeCondition.comparisonOperator = @"LT";
                    break;
                case NSLessThanOrEqualToPredicateOperatorType:
                    rangeCondition.comparisonOperator = @"LE";
                    break;
                default:
                    break;
            }
            [rangeCondition addAttributeValueList:attributeValueRangeKeyValue];
            [queryRequest.keyConditions setObject:rangeCondition forKey:[self rangeKeyForEntityName:request.entity.name]];
        }
        ...
        do {
            ...
            queryResponse = [self.ddb query:queryRequest];
            ...
            if (queryResponse.error == nil)
            {
                ...
                for (NSDictionary *item in queryResponse.items) {
                    if([item count] > 0)
                    {
                        DynamoDBAttributeValue *attributeValueHashKey = (DynamoDBAttributeValue *)[item objectForKey:[self hashKeyForEntityName:request.entity.name]];
                        NSString *hashKey;
                        
                        if(attributeValueHashKey.s != nil)
                        {
                            hashKey = attributeValueHashKey.s;
                        }
                        else if(attributeValueHashKey.n != nil)
                        {
                            hashKey = attributeValueHashKey.n;
                        }
                        
                        DynamoDBAttributeValue *attributeValueRangeKey = (DynamoDBAttributeValue *)[item objectForKey:[self rangeKeyForEntityName:request.entity.name]];
                        NSString *rangeKey;
...
- (BOOL)deleteObject:(NSSet *)deletedObjects error:(NSError **)error
{
    ...
    @try
    {
        ...
        {
            DynamoDBAttributeValue *hashAttributeValue = [self attributeValueFromObject:[managedObject valueForKey:[self hashKeyForEntityName:managedObject.entity.name]]];
            NSMutableDictionary *key = [NSMutableDictionary dictionaryWithObject:hashAttributeValue
                                                                              forKey:[self hashKeyForEntityName:managedObject.entity.name]];
            
            if ([self rangeKeyForEntityName:managedObject.entity.name] != nil) {
                DynamoDBAttributeValue *rangeAttributeValue = [self attributeValueFromObject:[managedObject valueForKey:[self rangeKeyForEntityName:managedObject.entity.name]]];
                NSMutableDictionary *rangeKey = [NSMutableDictionary dictionaryWithObject:rangeAttributeValue
                                                                                   forKey:[self rangeKeyForEntityName:managedObject.entity.name]];
                [key addEntriesFromDictionary:rangeKey];
...
- (NSString *)referenceObjectIdForCoreData:(id)hashKeyOrCompositeKey
{
    if([hashKeyOrCompositeKey isKindOfClass:[NSDictionary class]])
    {
        ...
        NSString *compositeKeyAsString = [NSString stringWithFormat:@"%@r%@", hashKey, rangeKey];
